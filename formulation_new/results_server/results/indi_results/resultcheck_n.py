...
def optimum_lam_nonlinear():
    ...
    lam =  pickle.load(open("../lambda_sweep_f_n/lam_LVAR.txt","rb"))
    cost_val = [0]*len(lam)
    ...
    for i in range(len(lam)):
        ...
        cost_val[i] = pickle.load(open("../lambda_sweep_f_n/val_lambda_"+str(np.round(lam[i],5))+"_.txt","rb"))

    minpos = cost_val.index(min(cost_val))
    optimum_lam = lam[minpos]
    
    cost_val_s = [0]*len(lam)

    for i in range(len(lam)):
        
        cost_val_s[i] = pickle.load(open("../lambda_sweep_f_n/val_lambda_"+str(np.round(lam[i],5))+"_s.txt","rb"))


    minpos_s = cost_val_s.index(min(cost_val_s))
    optimum_lam_s = lam[minpos_s]
    ...
    axis.plot(lam,cost_val_s,'-ro',label = "linear VAR_val sliced",linewidth=3)
    ...
    #plt.show()
    return optimum_lam,optimum_lam_s
