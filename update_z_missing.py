...
def update_z_missing(eta_z, z, A, alpha, w, k, b, t, m_data, z_tilde_data,hyperparam_nu):
    N,N,P = A.shape
    N,M = k.shape
    #for i in range(N):  # this way of formulation is wrong (loop should be inside backward)

    
    z,cost_missing,dTC_dZ = compute_gradients_z(z, A, alpha, w, k, b, t, m_data, z_tilde_data,hyperparam_nu)
